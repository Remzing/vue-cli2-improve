      ...
      <el-dialog
          ...
          <div style="padding:0">
            <div class="dis-flex warn-hint">
              ...
              <div>您确定要关闭此应用，一旦关闭成功，该客户无法正常进入该应用后台和小程序会提示该应用已关闭！</div>
            </div>
            <el-form :model="formData" label-position="top" status-icon :rules="rules2" ref="formData" label-width="100px" class="demo-ruleForm">
              <el-form-item  prop="reason">
                <label><span class="red-star">*</span>关闭理由：</label>
                <el-input type="textarea"  :rows="3" v-model="formData.reason" placeholder="请输入关闭理由"></el-input>
                <!-- <el-input placeholder="确认密码" type="password" v-model="formData.reason" auto-complete="off"></el-input> -->
              </el-form-item>
            </el-form>
          </div>
...
import { closeOrderXcx } from '@/api/order/xcxorder'
/*  */
export default {
  components: {
  ...
  methods: {
    ...
    confirmBtn(){
      ...
      closeOrderXcx(this.formData).then(response => {
