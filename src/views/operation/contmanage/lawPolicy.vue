    ...
    <el-row class="warp">
      <el-col :span="24" class="warp-main">
        <el-form class="edit-for" ref="formData" :model="formData" label-position="top" :rules="rules" label-width="120px">
          <el-form-item prop="law">
            <div style="line-height: 24px !important;">
              <quill-editor v-model="formData.law"
                            ref="myQuillEditor"
                            class="editer"
...
export default {
  ...
  data () {
    return {
      formData:{
        law:'',
        type:2  // 保存类型(1.服务协议 2.法律声明)
      },
      rules:{
        law: [
          { required: true, message: '请填写法律声明和隐私政策', trigger: 'blur' }
        ],
      },
      editorOption: {}
    };
  ...
  methods: {
    ...
    init(){
      serviceContractInit({}).then(response => {
        this.formData.law = response.data.law
      }).catch(error => {
