...
<template>
  <div style="height:100%;">
    ...
    <el-table :data="dataList"
      ...
      <el-table-column prop="bankname_kaihu" label="开户名"></el-table-column>
      <el-table-column prop="bankid_kaihu" label="开户账号"></el-table-column>
      <el-table-column prop="bankid_kaihu" label="开户银行"></el-table-column>
...
import { cashResultList } from '@/api/money/monCash'
...
export default {
      ...
      if (this.sidebar.thrLev ){
        this.sidebar.thrLev.forEach(ele => {
          if(ele.path === this.$route.path){
            ...
            console.log('dropOperArr-dropOper-----', dropOper, ele.children)
          }
        })
        console.log('dropOperArr-this.$route.path-----',this.$route.path)
      }
      return dropOper
    }
  },
  ...
  methods: {
     ...
     */
    fetchList(){
      cashResultList(this.formData).then(response => {
        let _data = response
