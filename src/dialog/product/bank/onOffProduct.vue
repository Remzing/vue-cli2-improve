      ...
      <el-dialog
          ...
          <div >
            <el-form :model="formData" label-position="top" status-icon :rules="rules2" ref="formData" label-width="100px" class="demo-ruleForm">
              <el-form-item  prop="is_putaway">
                <div><span class="red-star">*</span> 选择状态：</div>
                <el-radio-group  v-model="formData.is_putaway">
...
import { putawayProduct } from '@/api/product/publish'
/*  */
export default {
  components: {
  ...
  methods: {
    init(){
      ...
      this.formData.is_putaway =  this.singleItem.is_putaway||0
    ...
    confirmBtn(){
      ...
      putawayProduct(this.formData).then(response => {
